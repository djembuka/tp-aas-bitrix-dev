...
(function (exports, ui_vue3, local_vueComponents_controlChoice, ui_vue3_pinia) {
        ...
        if (item.controls) {
          item.controls.forEach(function (control) {
            if (!control.value && control.sub) {
              control.sub.forEach(function (s) {
                s.disabled = true;
              });
            }
          });
  ...
  var SubmitButton = {
    ...
    computed: {
      ...
      isDisabled: function isDisabled() {
        ...
        if (this.controlsBlock && this.controlsBlock.controls) {
          //required
          var requiredControls = this.controlsBlock.controls.filter(function (
            control
          ) {
          ...
          //width pattern
          var patternControls = this.controlsBlock.controls.filter(function (
            control
          ) {
        ...
        if (this.confirmDocsBlock && this.confirmDocsBlock.items) {
          ...
          var requiredConfirm = checked
            ? checked.controls.filter(function (control) {
                return control.required;
              })
            : undefined;
          confirmDocsFlag = requiredConfirm
            ? requiredConfirm.every(function (control) {
                return control.value &&
                  babelHelpers['typeof'](control.value) === 'object'
                  ? control.value.every(function (value) {
                      return value.val !== '';
                    })
                  : control.value !== '';
              })
    ...
    methods: {
      ...
      clickContinue: function clickContinue() {
        var control = Array.from(
          document.querySelector('.b-appeal-new-change-form form').elements
        ).find(function (elem) {
          var isRequired = elem.classList.contains('mx-input')
  ...
  var Application = {
    ...
    computed: _objectSpread$1(
      {},
      ui_vue3_pinia.mapState(formStore, [
        'hidden',
        'docsBlock',
        'controlsBlock',
        'confirmDocsBlock',
          ...
          babelHelpers.classPrivateFieldSet(
            ...
            ui_vue3.BitrixVue.createApp({
              ...
              mounted: function mounted() {
                ...
                formStore().confirmDocsBlock =
                  self.options.data.confirmDocsBlock;
                formStore().autosaveTimeoutId =
                  self.options.data.autosaveTimeoutId;
