/* eslint-disable */
(function (exports,ui_vue3,local_vueComponents_modalYesNo,local_vueComponents_modalAnyContent,local_vueComponents_controlChoice,local_vueComponents_buttonComponent,local_vueComponents_loaderCircle,local_vueComponents_messageComponent,ui_vue3_pinia) {
  'use strict';
  ...
  var formStore = ui_vue3_pinia.defineStore('form', {
    ...
    actions: {
      ...
      runDeleteComment: function runDeleteComment(commentId) {
        this.error = '';
        this.loading = true;
  ...
  var editFormStore = ui_vue3_pinia.defineStore('edit-form', {
    state: function state() {
      return {
        editLoading: false,
        editError: '',
    ...
    actions: {
      ...
      changeEditError: function changeEditError(error) {
      ...
      runSendEditForm: function runSendEditForm() {
        this.editError = '';
        this.editLoading = true;
        var d = dataStore();
        var formElem = document.querySelector("#".concat(dataStore().id, "EditForm"));
        var formData = new FormData(formElem);
        Object.keys(d.data).forEach(function (key) {
          formData.append(key, d.data[key]);
        });
  ...
  var controlsStore = ui_vue3_pinia.defineStore('controls', {
    ...
    actions: {
      changeEditControls: function changeEditControls(controls) {
  ...
  var Application = {
    ...
    methods: _objectSpread$2(_objectSpread$2(_objectSpread$2(_objectSpread$2({}, ui_vue3_pinia.mapActions(formStore, ['runGetComments', 'runGetForm', 'runSendForm', 'runDeleteComment', 'changeDeleteModalStateWatcher', 'changeComments', 'changeForm', 'changeLoading', 'changeError', 'changeActiveCommentId'])), ui_vue3_pinia.mapActions(editFormStore, ['runGetEditForm', 'runSendEditForm', 'changeEditModalStateWatcher', 'changeEditLoading', 'changeEditError'])), ui_vue3_pinia.mapActions(controlsStore, ['changeControls', 'changeEditControls', 'changeControlValue', 'runHints', 'setHints'])), {}, {
      ...
      clickEditModalYes: function clickEditModalYes() {
        var _this = this;
        this.runSendEditForm().then(function (response) {
          _this.changeEditLoading(false);
          _this.changeEditError('');
          _this.changeEditModalStateWatcher();
          return _this.runGetComments();
        }, function (response) {
          _this.changeEditLoading(false);
          _this.changeEditError(response.errors[0].message);
        }).then(function (response) {
          _this.changeLoading(false);
          _this.changeError('');
          _this.changeComments(response.data.comments);
        }, function (response) {
          _this.changeLoading(false);
      ...
      clickDeleteModalYes: function clickDeleteModalYes() {
        var _this2 = this;
        this.runDeleteComment(this.activeCommentId).then(function (response) {
          return _this2.runGetComments();
        }, function (response) {
      ...
      clickEdit: function clickEdit(commentId) {
        ...
        this.runGetEditForm(commentId).then(function (response) {
          ...
          _this3.changeEditLoading(false);
          _this3.changeEditError(response.errors[0].message);
        });
        this.changeEditModalStateWatcher();
      },
