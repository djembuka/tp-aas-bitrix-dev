...
export const AddEditForm = {
  ...
  computed: {
    ...
    sendButtonDisabled() {
      return this.currentBlocks.some((block) =>
        block.controls.some((control) => control.required && !control.value)
      );
    },
  },
  methods: {
    ...
    clickCancel() {
      ...
      setTimeout(() => {
        const capitalizedType =
          this.typeMode.charAt(0).toUpperCase() + this.typeMode.slice(1);
    ...
    async handleGroupSubmission() {
      ...
      const groupsResult = await this.runBitrixMethod('groupsQuestions', {
        uuid: this.uuid,
      });
    ...
    async handleQuestionSubmission() {
      ...
      const questionsResult = await this.runBitrixMethod('questions', {
        uuid: this.groupUuid,
      });
    ...
    async handleAnswerSubmission() {
      ...
      const answersResult = await this.runBitrixMethod('answers', {
